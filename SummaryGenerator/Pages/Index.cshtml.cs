...
namespace SummaryGenerator.Pages
{
    public class IndexModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostEnqueueAudioAsync(CancellationToken cancellationToken)
        {
            LoadPageData();

            if (UploadedMd is null || UploadedMd.Length == 0)
            {
                ModelState.AddModelError(nameof(UploadedMd), "Select a markdown file to upload.");
                return Page();
            }
            ...
            var fileName = $"{Guid.NewGuid():N}-{Path.GetFileName(UploadedMd.FileName)}";
            var uploadedMdPath = Path.Combine(uploadsRoot, fileName);

            try
            {
                await using (var stream = new FileStream(uploadedMdPath, FileMode.Create, FileAccess.Write, FileShare.None))
                {
                    await UploadedMd.CopyToAsync(stream, cancellationToken);
                }

                var task = _processingQueue.EnqueueAudio(uploadedMdPath, SelectedVoice);
                StatusMessage = $"Queued markdown file '{UploadedMd.FileName}' for audio generation with task ID {task.Id}.";
                _logger.LogInformation("Queued markdown upload {FileName} for audio as task {TaskId}.", UploadedMd.FileName, task.Id);
                return RedirectToPage();
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to queue uploaded markdown file {FileName}.", UploadedMd.FileName);
                ErrorMessage = $"Failed to queue file '{UploadedMd.FileName}': {ex.Message}";
                return RedirectToPage();
            }
        }

        public IActionResult OnPostGenerateAudioFromSummary(Guid taskId, string? selectedVoice)
        {
            try
            {
                var task = _processingQueue.EnqueueAudioFromSummary(taskId, selectedVoice);
                StatusMessage = $"Queued audio generation from summary task {taskId} as new task {task.Id}.";
                return RedirectToPage();
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to queue audio from summary {TaskId}.", taskId);
                ErrorMessage = $"Failed to queue audio generation: {ex.Message}";
                return RedirectToPage();
            }
        }

        public IActionResult OnGetDownloadAudio(Guid taskId)
        {
            if (!_processingQueue.TryGetTask(taskId, out var task) ||
                task?.Result is null ||
                !System.IO.File.Exists(task.Result.OutputAudioPath))
            {
                return NotFound();
            }

            var fileName = Path.GetFileName(task.Result.OutputAudioPath);
            var stream = new FileStream(task.Result.OutputAudioPath, FileMode.Open, FileAccess.Read, FileShare.Read);
        ...
        public IActionResult OnGetStatus()
        {
            var statuses = _processingQueue.GetAllTasks()
                .Select(task => new
                {
                    task.Id,
                    task.TaskType,
                    SourcePdfPath = task.SourcePdfPath ?? task.SourceMarkdownPath,
                    ...
                    task.ErrorMessage,
                    HasOutput = task.Result is not null && !string.IsNullOrWhiteSpace(task.Result.OutputMarkdownPath) && System.IO.File.Exists(task.Result.OutputMarkdownPath),
                    HasAudioOutput = task.Result is not null && !string.IsNullOrWhiteSpace(task.Result.OutputAudioPath) && System.IO.File.Exists(task.Result.OutputAudioPath)
                });
        ...
        private void LoadPageData()
        {
            ...
            var voices = _audioGenerator.GetAvailableVoices();
            if (!voices.Any(v => string.Equals(v, SelectedVoice, StringComparison.OrdinalIgnoreCase)))
            {
                SelectedVoice = voices.FirstOrDefault();
            }

            VoiceChoices = voices.Select(voice => new SelectListItem
            {
                Text = voice,
                Value = voice,
                Selected = string.Equals(voice, SelectedVoice, StringComparison.OrdinalIgnoreCase)
            }).ToList();
            ...
            TaskSummaries = _processingQueue.GetAllTasks()
                .Select(task => new TaskSummaryViewModel
                {
                    ...
                    SourceFileName = Path.GetFileName(task.SourcePdfPath ?? task.SourceMarkdownPath ?? string.Empty),
                    ModelName = task.Model?.Name ?? "-",
                    PromptProfileName = task.PromptProfileName ?? "-",
                    Status = task.Status.ToString(),
        ...
        public class TaskSummaryViewModel
        {
            ...
            public ProcessingTaskType TaskType { get; init; }
            ...
            public bool HasAudioOutput { get; init; }
        }
